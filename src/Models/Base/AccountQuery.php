 ...
 *
 * @method     ChildAccountQuery leftJoinDomain($relationAlias = null) Adds a LEFT JOIN clause to the query using the Domain relation
 * @method     ChildAccountQuery rightJoinDomain($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Domain relation
 * @method     ChildAccountQuery innerJoinDomain($relationAlias = null) Adds a INNER JOIN clause to the query using the Domain relation
 *
 * @method     ChildAccountQuery joinWithDomain($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Domain relation
 *
 * @method     ChildAccountQuery leftJoinWithDomain() Adds a LEFT JOIN clause and with to the query using the Domain relation
 * @method     ChildAccountQuery rightJoinWithDomain() Adds a RIGHT JOIN clause and with to the query using the Domain relation
 * @method     ChildAccountQuery innerJoinWithDomain() Adds a INNER JOIN clause and with to the query using the Domain relation
 *
...
abstract class AccountQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Models\Base\AccountQuery object.
     *
     * @param     string $dbName     The database name
     * @param     string $modelName  The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\Models\\Account', $modelAlias = null)
    {
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = AccountTableMap::getInstanceFromPool(null === $key || is_scalar($key) || is_callable([
                    $key,
                    '__toString',
    ...
    {
        $sql = 'SELECT id, name, created_at, updated_at FROM account WHERE id = :p0';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            AccountTableMap::addInstanceToPool($obj,
     ...
     *
     * @param \Models\Domain|ObjectCollection $domain     the related object to use as filter
     * @param string                          $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildAccountQuery The current query, for fluid interface
     */
    public function filterByDomain($domain, $comparison = null)
    {
        if ($domain instanceof \Models\Domain) {
            return $this
                ->addUsingAlias(AccountTableMap::COL_ID, $domain->getAccountId(), $comparison);
    ...
    {
        return $this->addUsingAlias(AccountTableMap::COL_UPDATED_AT, time() - $nbDays * 24 * 60 * 60,
    ...
    {
        return $this->addUsingAlias(AccountTableMap::COL_CREATED_AT, time() - $nbDays * 24 * 60 * 60,
            Criteria::GREATER_EQUAL);
    }
