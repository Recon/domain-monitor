...
use Models\UsersDomain;
use Models\UsersDomainQuery;
...
/**
 * This class defines the structure of the 'users_domain' table.
 *
 *
 *
 ...
 *
 */
class UsersDomainTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'Models.Map.UsersDomainTableMap';
     ...
     */
    const TABLE_NAME = 'users_domain';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\Models\\UsersDomain';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Models.UsersDomain';
     ...
     */
    const COL_USER_ID = 'users_domain.user_id';

    /**
     * the column name for the domain_id field
     */
    const COL_DOMAIN_ID = 'users_domain.domain_id';
    ...
    {
        // attributes
        $this->setName('users_domain');
        $this->setPhpName('UsersDomain');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\Models\\UsersDomain');
     ...
     *
     * @param mixed $value A \Models\UsersDomain object or a primary key value.
     */
    ...
    {
        return $withPrefix ? UsersDomainTableMap::CLASS_DEFAULT : UsersDomainTableMap::OM_CLASS;
    }
     ...
     * @return array           (UsersDomain object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = UsersDomainTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = UsersDomainTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + UsersDomainTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = UsersDomainTableMap::OM_CLASS;
            /** @var UsersDomain $obj */
            $obj = new $cls();
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = UsersDomainTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = UsersDomainTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(UsersDomainTableMap::COL_USER_ID);
            $criteria->addSelectColumn(UsersDomainTableMap::COL_DOMAIN_ID);
        } else {
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(UsersDomainTableMap::DATABASE_NAME)->getTable(UsersDomainTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(UsersDomainTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(UsersDomainTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new UsersDomainTableMap());
        }
    }
    ...
    public static function doDelete($values, ConnectionInterface $con = null)
    {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Models\UsersDomain) { // it's a model object
            ...
            foreach ($values as $value) {
                ...
                $criterion->addAnd($criteria->getNewCriterion(UsersDomainTableMap::COL_DOMAIN_ID, $value[1]));
                $criteria->addOr($criterion);
            }
        }

        $query = UsersDomainQuery::create()->mergeWith($criteria);
    ...
    {
        return UsersDomainQuery::create()->doDeleteAll($con);
    }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from UsersDomain object
        }
...
//
UsersDomainTableMap::buildTableMap();
